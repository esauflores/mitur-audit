...
## 10 corrective findings, PIE-scored
...
| 5 | **F-07** — HTML page sent uncompressed (50 KB on wire, ~25 KB with br) | 8 | 6 | 8 | **384** | Phase 1 |
| 6 | **F-04** — 1.7 MB of image transfer, no modern format | 8 | 7 | 6 | **336** | Phase 2 |
| 7 | **F-08** — 60 script requests (660 KB) — plugins each ship their own bundle | 8 | 7 | 5 | **280** | Phase 2 |
| 8 | **F-06** — Favicon is missing (404, 41 KB wasted per visit) | 9 | 3 | 10 | **270** | Phase 1 |
| 9 | **F-05** — 33 stylesheets + 41 `<link>` round-trips | 6 | 6 | 4 | **144** | Phase 2 |
| 10 | **F-09** — 2×404 + 1×401 broken-plugin errors | 7 | 2 | 8 | **112** | Phase 2 |

### Per-finding scoring rationale (selected)

- **F-12 (504)**: P=9 (1.2 s on mobile), I=8 (mobile amplification is the user's actual experience), E=7 (solved by fixing F-03 + F-08 — same fixes, biggest user-visible win is on mobile). The high E reflects "1 PR but 2 different findings depend on it."
...
### Phase 1 (week 1) — score ≥ 270, PIE top quartile

Five findings cluster in the top quartile. They share a common property: P ≥ 7 AND I ≥ 5 AND E ≥ 5. Ship all five in one week:

1. **F-12** (504) — fix the underlying F-03 + F-08, but report as "mobile TBT" for stakeholder visibility
...
5. **F-07** (384) — enable Cloudflare Brotli for HTML
6. **F-06** (270) — upload a real favicon

**Phase 1 outcome:** CLS 0.382 → ≤ 0.1, LCP 6.6 s → 2.5–3.5 s, TBT 300 ms → 150–200 ms (Lighthouse) / 600–800 ms (real mid-tier Android), favicon 404 → 200, HTML 50 KB → ~25 KB. Estimated impact: 4 of 5 CWV move from "poor" to "needs improvement" or better.
...
- **F-05** (144) — concatenate CSS into a single bundle
- **F-09** (112) — deactivate Popup Maker plugin (eliminates 2×404 + 1×401)

**Phase 2 outcome:** image transfer 1.7 MB → 0.85 MB, script requests 60 → 1, stylesheet round-trips 41 → 1, broken-plugin errors 3 → 0.
...
PIE is stable in the top quartile. Below the median, scores cluster and a one-point shift in any dimension moves findings around. Specifically:
...
- F-06 (270) scores high because of the multiplicative 9×3×10 — but those numbers reflect that the user *doesn't care*, the fix is trivial, and the value is *cleanliness*. If you weight "user impact" alone, F-06 would drop to F-09's tier.

This is the value of PIE over RICE for a single-page audit: the multiplicative formula makes trivial-impact findings like F-06 (favicon 404) drop below the top quartile because of low Importance, even though Ease is 10.
...
| MITUR PIE top quartile (PIE) | AP News consensus top tier (RICE / ICE / WSJF) | Overlap |
| --- | --- | --- |
...
The "top tier" is the same on both audits: fix the deferred scripts, fix the CLS, fix the LCP image. PIE and the AP News triangulation independently converge. That gives me confidence the recommendations are robust to methodology.
...
- **F-11** — Zero third-party vendors on the homepage (clean attack surface)
