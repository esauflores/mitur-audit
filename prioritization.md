...
- **F-18 (432)**: P=8 (no critical-CSS pipeline today; after PurgeCSS, 244 KB → 50 KB), I=9 (every visitor on every page pays the render-blocking cost; FCP impact 0.5-1.0 s), E=6 (PurgeCSS via build step + custom `wp_enqueue_style` filter; 1-2 weeks of work).
...
- **F-19 (294)**: P=7 (frame-chart data quantifies the F-12/F-14 tail — 2 dropped frames in 5s), I=6 (1 dropped frame in 5s is barely perceptible), E=7 (no new action; the fix is in F-12/F-14). Higher score than the per-impact alone because the frame chart is a "show your work" artifact for the existing findings.
...
- **F-20 (48)**: P=4 (saves ~6-8 MB GPU memory; no CWV impact), I=3 (no user-perceptible jank today — page is 60 fps even with the wasteful patterns), E=4 (need to modify plugin CSS or open tickets with plugin authors). Bottom of the table by design — the patterns are wasteful but not user-perceptible.

## Phase plan (PIE-ranked)

### Phase 1 (week 1) — score ≥ 384, top cluster

Eight findings cluster in the upper end of the table. They share a common property: P ≥ 7 AND I ≥ 5 AND E ≥ 5 (a balanced high-scoring profile — all three dimensions need to be ≥ 5 to break 384 with PIE). Ship all eight in one week:
...
F-20 documents a real waste — 6 of 7 `will-change` declarations are block-editor admin CSS bleeding to public, plus 2 `translate3d(0,0,0)` from Smart Slider 3 — but the page is still 60 fps. There's no user-perceptible jank to fix today. PIE correctly down-ranks it — low Potential (small GPU memory win, no CWV) × low Importance (no user impact) × low Ease (need to modify plugin CSS or open tickets) = 48. **The right call: ship it last, only as good housekeeping.** If the team fixes F-15 (drop block-editor admin CSS), 6 of the 7 `will-change` declarations go away for free; the remaining 2 require plugin-author cooperation.

### Why F-09 ranks second-to-last (112) despite being easy

F-09 is a real bug, but the user impact is negligible (1 KB of transfer waste, no visible behavior). PIE correctly down-ranks it — high Potential (clean diff) × low Importance (no user pain) × high Ease (deactivate plugin) = middling score. The right call: ship it late, not first.
...
- F-14 (405) vs F-15 (392) vs F-18 (432): close cluster, all about the "stop shipping code nobody uses" family. F-18 has the highest I (9 = every visitor on every page, render-blocking CSS) but lower E (PurgeCSS build pipeline). F-15 has highest E (one-liner `wp_dequeue_style`). F-14 has highest P (51 scripts → 15 saves the most bytes). Any of these 3 is defensible as #1 in this sub-cluster.
